...
Python 엔진 테스트에는 실제 yt-dlp HTTP 다운로드, 영상/오디오 두 파일 검증, 외부 프로세스 호출 방지, 화질 상한·코덱·확장자 선택·원본 포맷 폴백, 자막 선택, 임의 yt-dlp 인수 파싱과 우선순위, SHA-256 검증 업데이트, 취소, JavaScriptCore 제공자 등록과 플랫폼별 확장 모듈 배치가 포함됩니다.
...
- 기본 동영상 모드는 H.264 MP4를 우선합니다. 해당 조합이 없으면 같은 화질 상한 안에서 영상과 소리가 함께 든 MP4 또는 다른 원본 컨테이너로 자동 폴백하며 실제 확장자를 그대로 보존합니다. 화질 선택은 업스케일이 아닙니다.
- `원본 포맷`을 켜면 변환·리먹스 없이 영상+오디오가 함께 든 가장 좋은 단일 원본 스트림을 저장합니다. 오디오 모드에서는 가장 좋은 오디오 원본을 확장자 그대로 저장합니다.
- `yt-dlp 기본 선택`은 앱의 포맷·화질·확장자 선택을 건너뛰고 yt-dlp 기본 형식 선택을 사용합니다. 앱 통합에 필요한 출력 경로·로그·취소 설정은 유지되며, 기본 선택이 분리 스트림 병합을 요구하면 iOS의 외부 FFmpeg 부재로 실패할 수 있습니다.
- 영상과 오디오가 분리되어 있으면 각각 내려받고 AVFoundation으로 합칩니다. FFmpeg 실행 파일은 사용하지 않습니다.
- 오디오는 AAC/M4A를 우선하고 없으면 WebM/Opus 등 다운로드 가능한 원본 오디오로 폴백합니다. DRM, 로그인 쿠키, 재생목록 및 진행 중인 라이브는 지원하지 않습니다.
...
- `yt-dlp 인수` 칸이 비어 있지 않으면 해당 문자열을 `shlex`로 나눈 뒤 yt-dlp 자체 명령줄 파서로 그대로 해석합니다. 별도의 인수 허용 목록은 두지 않습니다. 이 모드에서는 메인 화면의 MP4/M4A, 화질, 원본 포맷, `yt-dlp 기본 선택`과 앱의 확장자·자막 선택을 사용하지 않습니다.
- 앱은 직접 인수 모드에서도 링크 전달, 진행률/로그 연결, 취소, 기본 저장 위치와 최종 결과 파일을 보관함으로 가져오는 과정만 관리합니다. 사용자가 `-o`/`-P`를 지정하면 해당 yt-dlp 설정을 우선합니다. 여러 미디어 파일을 만드는 인수, 파일을 만들지 않는 인수, FFmpeg나 외부 실행 파일을 요구하는 인수는 iOS 환경 또는 현재 보관함 구조 때문에 다운로드 후 가져오기에 실패할 수 있습니다.
...
13. 직접 yt-dlp 인수 입력 시 메인 옵션 무시 확인, `-f`/`-o`/`-P`/`-t` 등 다양한 인수, 자막·원본 확장자·`원본 포맷`·`yt-dlp 기본 선택` 동작 확인
