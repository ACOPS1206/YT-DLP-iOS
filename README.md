...
Linux 환경에서 Python 엔진과 패키징 테스트 19개가 통과했습니다. 테스트에는 실제 yt-dlp HTTP 다운로드, 영상/오디오 두 파일 검증, 외부 프로세스 호출 방지, 화질 상한·코덱·확장자 선택·원본 포맷 폴백, 자막 선택, 추가 인자 차단, SHA-256 검증 업데이트, 취소, JavaScriptCore 제공자 등록과 플랫폼별 확장 모듈 배치가 포함됩니다.
...
13. 자막 언어 우선순위, 자동 자막 끄기, 원본 확장자 지정, `원본 포맷`·`yt-dlp 기본 선택` 토글 및 허용되지 않은 추가 인자 오류 확인
