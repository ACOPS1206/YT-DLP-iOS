...
final class SharingAndActivityTests: XCTestCase {
    ...
    func testShareDeepLinkPreservesDownloadOptions() throws {
        ...
        XCTAssertEqual(components.queryItems?.first(where: { $0.name == "quality" })?.value, "1440")
        XCTAssertNil(components.queryItems?.first(where: { $0.name == "original" }))
